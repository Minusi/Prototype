...
UActorConstraintMarker* UScaleStartCommand::ActorConstraintMarker = nullptr;

UScaleStartCommand::UScaleStartCommand()
{
	VP_CTOR;
...
void UScaleStartCommand::ExecuteIf()
{
	if (IsValid(ActorConstraintMarker) == false)
	{
	...
	if (IsValid(ScaleInfo.SelectedGizmoComponent))
	{
	...
	for (const auto& it : Constraints)
	{
		if (it->CheckConstraint(Target) == true)
		{
			...
			for (int i = 0; i < ScaleInfo.SelectedGizmoComponent->GetAxisTypes().Num(); ++i)
			{
				Transformer->ScaleObject(Target.Target, EditorGizmo->
					GetAxisDirection(ScaleInfo.SelectedGizmoComponent->GetAxisTypes()[i]), 1, 1);
				VP_LOG(Warning, TEXT("작동이가 존재하지않습니다."));
			}
...
void UScaleStartCommand::InitActorCommand(FActorConstraintInfo TargetInfo)
{
	Target = TargetInfo;
}
