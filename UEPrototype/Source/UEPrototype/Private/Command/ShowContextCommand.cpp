...
UActorConstraintMarker* UShowContextCommand::ActorConstraintMarker = nullptr;

AContextor* UShowContextCommand::Contextor = nullptr;

UShowContextCommand::UShowContextCommand()
{
	VP_CTOR;
...
void UShowContextCommand::ExecuteIf()
{
	if (IsValid(Contextor) == false)
	{
	...
	for (const auto& it : Constraints)
	{
		if (it->CheckConstraint(Target) == true)
		{
			...
			Contextor->ShowContext(Target.Target);
			VP_LOG(Warning, TEXT("ShowContexCommand는 실행된다..."));
			return;
		}
	}
}

void UShowContextCommand::InitActorCommand(FActorConstraintInfo TargetInfo)
{
	Target = TargetInfo;
}
