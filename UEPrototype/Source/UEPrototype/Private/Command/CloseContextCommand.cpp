...
UActorConstraintMarker* UCloseContextCommand::ActorConstraintMarker = nullptr;

AContextor* UCloseContextCommand::Contextor = nullptr;

UCloseContextCommand::UCloseContextCommand()
{
	VP_CTOR;
	...
	if ((ActorConstraintMarker != nullptr && ActorConstraintMarker->IsValidLowLevel()) &&
		(Contextor != nullptr && Contextor->IsValidLowLevel()))
	{
		VP_LOG(Log, TEXT("ShowContext의 멤버가 유효하다네요?"));
		return;
	}
	VP_LOG(Log, TEXT("ShowContext의 멤버가 유효하지 않다네요?"));
	...
	Contextor = AContextor::GetGlobalContextor();



	/* 초기화된 객체들에 대한 유효성 검사를 실행합니다 */

	if (IsValid(Contextor) == false)
	{
		VP_LOG(Warning, TEXT("Contextor가 유효하지 않습니다.."));
		return;
	}
...
void UCloseContextCommand::ExecuteIf()
{
	if (IsValid(Contextor) == false)
	{
		VP_LOG(Warning, TEXT("명령을 실행하는데 Contextor가 유효하지 않습니다.."));
		return;
	}
	...
	for (const auto& it : Constraints)
	{
		if (it->CheckConstraint(Target) == true)
		{
			...
			Contextor->CloseContext(Target.Target);
			VP_LOG(Warning, TEXT("ClearContexCommand는 실행된다..."));
			return;
		}
	}
...
void UCloseContextCommand::InitActorCommand(FActorConstraintInfo TargetInfo)
{
	Target = TargetInfo;
}
